...
def init_redis(app):
    ...
    redis_conf = {
        key: value for key, value in redis_state.items() if key in redis_conf_keys
    }
...
def record_timer(app, interval):
    ...
    with app.app_context():
        try:
            ...
            in_time = time.time()
            target_time = int(
                int((time.time()) / ONE_MINUTE_SECONDS + 1) * ONE_MINUTE_SECONDS
            )
...
def query_flask_state():
    ...
    try:
        ...
        if not isinstance(b2d, dict):
            logger.warning(ErrorMsg.DATA_TYPE_ERROR).get_msg(
                ".The target type is {}, not {}".format(
                    dict.__name__, type(b2d).__name__
                )
            )
        ...
        return make_response_content(
            ErrorResponse(MsgCode.UNKNOWN_ERROR), http_status=500
        )
