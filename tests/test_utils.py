...
def test_auth_method(app):
    ...
    assert error_request_method == json.loads(str(get_response.data, "utf-8")).get(
        "msg"
    )
...
def test_format_sec():
    ...
    for key in test_type_list:
        try:
            ...
            assert (
                t.__str__()
                == "Data type format error. The target type is int, not %s" % key
            )
...
def test_format_address():
    ...
    for key in test_type_list:
        try:
                ...
                raise TypeError(
                    ErrorMsg.DATA_TYPE_ERROR.get_msg(
                        ". The target type is {}, not {}".format(
                            str.__name__, type(test_type_list.get(key)).__name__
                        )
                    )
                )
        except TypeError as t:
            ...
            assert (
                t.__str__()
                == "Data type format error. The target type is str, not %s" % key
            )
            assert isinstance(t, TypeError)
    ...
    for address in test_address_list:
        try:
            if (
                ...
                or address[: Constant.MIN_ADDRESS_LENGTH - 1]
                != format_conf.DB_URL_HEADER
            ):
                raise ValueError(
                    ErrorMsg.ERROR_ADDRESS.get_msg(". Error Sqlite url: %s" % address)
                )
        except ValueError as v:
            target_address_error_count += 1
            assert (
                v.__str__()
                == "Incorrect address format, Set the format to: sqlite:///path. Error Sqlite url: %s"
                % address
            )
    ...
    for address in correct_address_list:
        ...
        address_path = (
            address[Constant.MIN_ADDRESS_LENGTH - 1 :][:index] if index != -1 else "./"
        )
        try:
            ...
            assert (
                v.__str__()
                == "Path has no access, make sure you have access to the path. No access path: %s"
                % address
            )
